...
const fetcher = async (url: string) => {
  const res = await fetch(url, {
    cache: "no-store",
    credentials: "include", // ensure cookies are sent so the server can read the session
...
export default function Navigation() {
  ...
  useEffect(() => {
    ...
    const { data: sub } = supabase.auth.onAuthStateChange(() => {
      mutate() // refetch /api/auth/session
    })
    return () => {
  ...
  const handleSignOut = useCallback(async () => {
    ...
    await mutate({ user: null, profile: null }, false) // optimistic update
    router.refresh()
  }, [mutate, router])
